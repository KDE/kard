...
Kard::Kard() : KMainWindow( 0, i18n("Kard") ), m_view(new KardView(0, this))
{
    // Create a status bar
    ...
    QWhatsThis::add( m_numCombo, i18n( "Choose the level of difficulty by setting the number of cards from 4 (very easy) to 24 (more difficult)" ) );
    
    //adding themes combobox
    QStringList themesList;
    themesList += i18n("colors");
    themesList += i18n("colors & names");
    ...
    QWhatsThis::add( m_themeCombo, i18n( "You can choose here the theme for the backs of the cards" ) );
...
void Kard::slotUpdateTheme()
{
    ...
    switch (KardSettings::theme()) {
        case KardSettings::EnumTheme::colors:
            if (m_view->theme == "names" || m_view->theme=="house" || m_view->theme=="opposites"|| m_view->theme=="syllables" || m_view->theme=="animals") 
                boardChanged = true;
            m_view->theme = "colors";
            break;
             case KardSettings::EnumTheme::names:
            if (m_view->theme=="colors" || m_view->theme=="house" || m_view->theme=="opposites"|| m_view->theme=="syllables" || m_view->theme=="animals") 
                boardChanged = true;
            m_view->theme = "names";
            break;
        case KardSettings::EnumTheme::house:
            if (m_view->theme == "names" || m_view->theme=="colors" || m_view->theme=="opposites" || m_view->theme=="syllables" || m_view->theme=="animals" || m_view->theme=="food")
                boardChanged = true;
            m_view->theme = "house";
            break;
        case KardSettings::EnumTheme::opposites:
            if (m_view->theme == "names" || m_view->theme=="house" || m_view->theme=="colors"|| m_view->theme=="syllables" || m_view->theme=="animals" || m_view->theme=="food") 
                boardChanged = true;
            m_view->theme = "opposites";
            break;
        case KardSettings::EnumTheme::syllables:
            if (m_view->theme == "names" || m_view->theme=="house" || m_view->theme=="colors" || m_view->theme=="opposites" || m_view->theme=="animals" || m_view->theme=="food") 
                boardChanged = true;
            m_view->theme = "syllables";
            break;
        case KardSettings::EnumTheme::animals:
            if (m_view->theme == "names" || m_view->theme=="house" || m_view->theme=="colors" || m_view->theme=="opposites" || m_view->theme=="syllables" || m_view->theme=="food") 
                boardChanged = true;
            m_view->theme = "animals";
        break;
        case KardSettings::EnumTheme::food:
            if (m_view->theme == "names" || m_view->theme=="house" || m_view->theme=="colors" || m_view->theme=="opposites" || m_view->theme=="syllables" || m_view->theme=="animals") 
                boardChanged = true;
            m_view->theme = "food";
            break;
    }
